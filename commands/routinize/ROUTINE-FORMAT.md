...
## Local Extension Format (`.omp/extensions/<slug>.ts`)

When routinizing an interactive workflow into a zero-token local harness extension, follow the canonical modular extension shape:

### Extension Skeleton

```typescript
// .omp/extensions/<slug>.ts
// Local extension generated by /routinize
...
export default function (pi: ExtensionApi): void {
  pi.registerCommand("<slug>", {
    description: "<description inferred from task>",
    ...
    handler: async (args: string, ctx) => {
      ctx.ui?.notify?.("Executing <slug>...", "info");
...
When a markdown skill under `.omp/skills/<name>/SKILL.md` is purely procedural (e.g. running a static build sequence or specific CLI flags):

1. **Target Replacement**: Scaffold either a shell routine (`scripts/routines/<name>.sh`) or a local extension (`.omp/extensions/<name>.ts`).
2. **Token Economics**: Calculate estimated prompt token savings (~300-800 tokens).
3. **Safety Gate**: The markdown skill file is only deleted upon explicit per-item user confirmation (`approve`), ensuring creative/conversational skills (`tdd`, `grilling`) are never pruned.
