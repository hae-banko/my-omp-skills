...
- Save to `{project_dir}/report.md` and `{project_dir}/summary.md` (both written in the same single pass)
...
3. **Shared context** — repo anchor facts that recur across many findings (the same source symbol, upstream issue number, or convention cited over and over), surfaced once instead of per item. Driven by the extensible `SHARED_CONTEXT_PATTERNS` regex list in the script; only anchors mentioned in ≥2 items render, most-repeated first. Projects that mention none of the anchors get no section.
4. **Action Plan** — a table of every item (`# | Finding | Severity | Priority | Effort | Affected component`) sorted by priority then severity then item number, so the report opens with "what to fix first". Long affected-component values are truncated in the table; full value lives in the item meta line.
5. **Table of Contents** — every item: number, name (anchor link), plus short badges (priority, effort) when present. Never truncate names; the Action Plan carries the summary fields.
6. **Findings** — one section per item:
...
7. **Sources** — deduped appendix: every distinct URL with the item numbers it grounds. Bare domains (`w3.org/TR/WCAG22`) are matched as sources; file paths (`src/foo.ts`, `README.md`) and mid-word truncations (`CustomMessagePayload.co` inside `.content`) are not.
8. **Unresolved Fields & Attempts** — per-item provenance for uncertain/empty fields, from `_attempts` (see requirement 7 below).

The same run also writes `{project_dir}/summary.md`, an **accessible digest** of the report: the header stats and Executive Summary verbatim, plus an Action Plan table of every item (`# | Finding | Severity | Priority | Effort | Problem`) where **Problem** is the item's one-line problem statement (the `ux_issue` one-liner, truncated to ~140 chars with `…`), and a short **Notes** footer pointing at `report.md`. The digest has no `<details>` blocks and no per-item bodies — it is readable without ever opening `report.md`.
...
- `{project_dir}/summary.md` — accessible summary digest (written by the same run: header stats + executive summary + Action Plan with a one-line Problem column + Notes footer)
