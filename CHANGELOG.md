...
## v0.75.1 — Command UX Invariant: Zero Think, Zero `--`, Zero Editor Echo (ADR-0008)

- **No more LLM echo in the input box.** The root cause of the `(QUICK)` / `(RAW)` / `(EMBEDDED)` repaint when running `/council` (and any other command) was `pi.sendUserMessage("/council ...")` echoing the user's typed text back into the editable prompt buffer. `src/council/council.ts:runCouncilCommand` now routes the agent turn through `pi.sendMessage({ deliverAs: "nextTurn", triggerTurn: true })` and **never** calls `pi.sendUserMessage` after the user typed. ADR-0008 §3.
- **No more sub-mode stamps in the status bar.** Status text was leaking routing metadata like `Council: QUICK (default-triad) deliberating on ...` into the user-facing status bar. Replaced with a clean `Council deliberating on "<topic>"` line. The verdict receipt card content was also cleaned of the `(QUICK)` / `(EMBEDDED)` decorations. ADR-0008 §4.
- **Zero `--` prefix in user-facing flag/keyword syntax.** `commands/council.md` and `skills/council/SKILL.md` now document every flag as a bare keyword (`quick`, `deep`, `save`, `overlay`, `verbose`, `ml`, `embedded`, `ee`, `hardware`). The arg parser still accepts `--quick` as a tolerant synonym for muscle memory, but the prose UX is bare form only.
- **`skipAgentTurn` gate.** Added `skipAgentTurn: boolean` to `CommandSpec`. `runDefaultHandler` in `src/index.ts` honors the flag — when true, it emits no `pi.sendUserMessage`, no hidden workflow body, and no follow-up receipt card. Pure-data display commands (`/timeline`, `/tilt`, `/reference`, `/clarify debug`, `/hindsight status`, `/record --recent`, `/pitfall --recent`, `/omp-setup status`, `/omp-handoff status`) should set this flag in a follow-up sweep; the council handler already routes via the new `nextTurn` channel.
- **`/council edit` (alias `/council config`) opens `.omp/council.yaml` in the built-in TUI editor** via `ctx.ui.editor`. Auto-scaffolds the config on first run, validates the YAML on save, and reloads the council configuration in memory.
- **`/council` interactive triad selector.** Running `/council` with no args in a TUI session opens an interactive selector (`ctx.ui.select`) listing every built-in + custom triad with the active default starred (`★ default`). Includes an `Edit .omp/council.yaml` option that launches `runCouncilEditSubcommand` inline. Adds `editor`, `select`, and `input` to `CommandContext.ui` in `src/core/api.ts`.
- **ADR-0008 + AGENTS.md §9 invariant.** Project-level constraint enforcing all six rules (no think, no echo, no `--`, autocomplete-everything, status-is-for-user, default-verbosity-silent-card). Cited from code review and every new command.

## v0.75.0 — Unified Native TUI Card Architecture, Semantic Intent, & Council Streamlining (SPEC-003 / ADR-0007)

- **Council Project Configuration & Default Selection (`.omp/council.yaml`)** — Added top-level `default: <preset>` and `defaultMode: <mode>` configuration options in `.omp/council.yaml`. Projects can designate their default council preset (e.g. `default: embedded` or `default: custom-team`), automatically selected when typing `/council <topic>` with zero flags or subagent discovery friction.
- **Arbitrary & Flexible Participant Counts ($N \ge 1$)** — Lifted the 3-participant constraint across `src/council/council.ts`. Projects can configure custom councils with any participant count ($N \ge 1$), supported by dynamic scaling for consensus invariants ($100\%$ unanimous across all $N$) and majority recommendations ($\ge 60\%$).
- **Silent-by-Default Invariant & Execution Contract** — Subagents are strictly barred from printing preliminary plans, persona drafting steps, or commentary in chat before dispatching the `task` batch (`SILENT-BY-DEFAULT DIRECTIVE`). `runCouncilCommand` directly injects a structured JSON execution contract with resolved system prompts, roles, and tool allow-lists.
- **Minimal Discussion & `verbose` Keyword** — Enforced concise 3–5 bullet point executive summaries after the verdict card, suppressing intermediate debate transcripts and critiques unless `verbose` is explicitly specified. Added `verbose` to CLI argument parsing, autocompletions (`src/index.ts`), and documentation.
- **Unified Card Engine (`src/core/card.ts`)** — Replaces hardcoded ANSI string buffers and rigid 76-column box drawing with a centralized native `@oh-my-pi/pi-tui` `Box` adapter (`createTuiCard`). Dynamically binds to Oh My Pi theme background tokens (`toolSuccessBg`, `toolPendingBg`, `toolErrorBg`, `customMessageBg`) and border tokens (`borderAccent`, `success`, `warning`, `error`, `borderMuted`), rendering with native rounded corners (`╭`, `╮`, `╰`, `╯`) and responsive width scaling across any terminal size.
- **Semantic Intent Model (`SemanticCardIntent`)** — Decouples card status from concrete terminal escape codes via abstract intents (`"neutral"`, `"active"`, `"success"`, `"warning"`, `"danger"`, `"accent"`), ensuring proper contrast in both light and dark themes (`theme.isLight`).
- **Responsive Dividers (`ResponsiveDividerLine`)** — Automatically expands section rules (`├─── TITLE ───┤`) to 100% of the active terminal viewport width.
- **Pioneer Subsystem Migrations (Phase 1)**:
  - **Council Verdicts (`src/council/council.ts`)** — `/council` verdicts render via `createCouncilVerdictCard` with consensus-aware semantic tinting, badges, stacked headers, and interactive keybinding action hints (`Enter: Run /implement`, `s: Save Record`, `Esc: Dismiss`).
  - **Tilt-O-Meter & Swear Jar (`src/features/tilt.ts`)** — `/tilt` renders via `createTiltCard` with tier-based rage archetypes, dynamic vibe meters, profanity breakdown bar charts, global repository leaderboards, and defensive git-push lock indicators.
  - **Timeline Digest (`src/features/timeline.ts`)** — `/timeline` renders via `createTimelineCard` with chronological event feeds, badge metadata, and viewport-adaptive event lines.
- **Headless Test Stubs (`scripts/stubs/pi-tui.ts`)** — Expanded `@oh-my-pi/pi-tui` test stubs to include full `Box`, `BoxBorder`, and word-wrapping `Text` components matching Oh My Pi's layout engine for deterministic headless testing.
- **Unit Test Suite (`tests/card.test.ts`)** — Added dedicated unit test suite covering intent token resolution, theme normalisation, responsive divider rendering, and full-width viewport scaling, integrated into `scripts/selftest.ts`.

## v0.74.1 — Re-land `installCouncilVerdictRenderer` Container fix (regression of v0.72.1)

- **Bug fix (`src/council/council.ts`)** — v0.72.1's CHANGELOG documented wrapping each line of the verdict card in a `Container` of `Text` widgets, but the actual source still returned the raw ANSI string from `renderCouncilVerdictCard(...)`. As a result every `omp --resume` of a session containing a `council-verdict` message crashed the render loop with `TypeError: t[i].render is not a function`. The renderer now splits the card string by `\n`, builds a `Container`, and adds one `Text(line, 0, 0)` child per line — matching the established pattern (`audit-card`, `ticket-breakdown`, `triage-status`, `research-*`, `timeline-digest`, `tilt-meter`, `clarify-debug`, `hindsight`, `knowledge-*`).
- **Stub fix (`scripts/stubs/pi-tui.ts`)** — Added `Container.render(width): string[]` (delegates to `children.render(width)`) and `Text.render(width): string[]` (respects `paddingX`/`paddingY`) so the selftest exercises the real harness render loop headlessly and catches future regressions of this class of bug.
- **Regression test (`tests/features.test.ts`)** — Inverted the prior `typeof === "string"` assertion (which was the v0.72.1 bug's smoking gun) into a positive Component-shape check: (a) the renderer must NOT return a string, (b) it must return an object with a callable `.render(width)`, (c) iterating all registered renderers and calling `.render(width)` — the exact harness pattern — must succeed and produce lines containing `COUNCIL VERDICT`, (d) every rendered line must respect the 76-column ANSI invariant. Re-running the test against the buggy code reproduces `TypeError: out.render is not a function`.
...
- **Relative-link contract preserved** — lessons still link the shared stylesheet as `../assets/<file>` (the path from inside the lessons directory is unchanged); the command body now documents the new relative paths for links to `reference/` documents and from reference documents to components.
