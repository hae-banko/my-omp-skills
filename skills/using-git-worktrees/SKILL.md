...
If the current session exposes a native worktree tool or a `/worktree` command, use it and skip to Step 2 — native tools handle directory placement, branch creation, and cleanup automatically, and `git worktree add` alongside them creates phantom state the harness can't manage. omp ships no such tool today, so this branch usually does not apply; fall through to Step 1b.
...
| "`git worktree add` is quicker than hunting for a native tool" | A native tool owns placement, branching, and cleanup. Bypassing it creates phantom state the harness can't manage — but omp has no such tool, so this rationalization only applies if one appears. |
